...
/* Allocate static memory for a string
   Note: cfree should be called to free allocated memory afterwards !! */
...
void vmc_skipUntil(FILE *fp,char *terminator) {
  ...
  if (lexControlChanged) {
    cfree(vmc_lexControl.single);
    cfree(vmc_lexControl.space);
...
int *vmc_getVM(void)
{
  int code,byte,page;
  char *cp;
  /* VMS does not give up memory when cfree() is called.  It seems to hang
