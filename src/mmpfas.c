...
void interactiveMatch(long step, long maxEssential)
{
  ...
  for (stmt = 1; stmt < g_proveStatement; stmt++) {
    ...
    // See if the maximum number of requested essential hypotheses is exceeded
    if (maxEssential != -1) {
      essHypCount = 0;
...
nmbrString *replaceStatement(long replStatemNum, long prfStep,
    ...
    // instead of whole proof, for faster speed (used by MINIMIZE_WITH).
    flag subProofFlag, 
  ...
  flag reenterFFlag;
  // Flag that replacement hypothesis under consideration has dummy variables
  flag dummyVarFlag; 
  ...
  while (1) { // Try all possible unifications
    ...
    for (hyp = 0; hyp < schReqHyps; hyp++) {
      ...
      // Scan all known steps of existing subproof to find a hypothesis match
          ...
          if (hasDummyVar) {
            ...
            if (hyp < schEHyps) {
              ...
              tmpFlag = unifyH(hypMakeSubstList[hypSortMap[hyp]],
                  ...
                  // (Remember: 1 = false, 2 = true in hypReEntryFlagList)
                  hypReEntryFlagList[hypSortMap[hyp]] - 1);
              // This should be a trivial unification, so it should never fail
      ...
      if (!foundTrialStepMatch) {
        ...
        // Go back one iteration (subtract 2 to offset end of loop increment).
        hyp = hyp - 2;
      } // if (!foundTrialStepMatch)
    } // next hyp
...
/*E*/                0, // explicitTargets
/*E*/                0 // statemNum, used only if explicitTargets
/*E*/                ), NULL));
...
vstring getIndepKnownSteps(long proofStmt, long refStep)
{
  ...
  for (prfStep = proofLen - 2; // next to last step
      prfStep >= 0;
      prfStep--) {
...
nmbrString *expandProof(
  ...
  // pntrNSpace initializes to null nmbrStrings 
  ...
  // Restart after every expansion (to handle nested
...
char checkStmtMatch(long statemNum, long step)
{
  ...
  // (The speedup is only done for .target; the .user is assumed to be infrequent.)
  // First symbol
...
nmbrString *proveFloating(const nmbrString *mString, long statemNum, long maxEDepth,
    ...
    flag noDistinct, // 1 means don't try statements with $d's
    // 1 means to override usage locks, 2 means to override
    // silently (for web-page syntax breakdown in mmcmds.c).
    flag overrideFlag,
    flag mathboxFlag) {
...
/*E*/   ": ", nmbrCvtMToVString(scheme), NULL));
      ...
      // Make substitutions into each hypothesis, and try to prove that hypothesis.
      free_nmbrString(proof);
      breakFlag = 0;
      for (hyp = 0; hyp < schReqHyps; hyp++) {
...
/*E*/   nmbrCvtMToVString(hypList[hypOrdMap[hyp]]), NULL));
        makeSubstPtr = makeSubstUnif(&tmpFlag, hypList[hypOrdMap[hyp]],
            stateVector);
        // No dummy vars. should result unless bad $a's
        ...
        // Deallocate in case this is the 2nd or later pass of main unification.
      ...
      if (getMarkupFlag(stmt, USAGE_DISCOURAGED)) {
        switch (overrideFlag) {
          ...
          // Accept override silently (in mmcmds.c syntax
...
INLINE flag quickMatchFilter(long trialStmt, const nmbrString *mString,
  ...
  // Speedup:  make sure all constants in scheme are in instance (i.e. mString)
    ...
    flag allowGrowthFlag)
{
  ...
  nmbrString *mString; // Pointer only; not allocated
  // Pointer only; not allocated; however initialize for nmbrLen function
  // before it's assigned.
  nmbrString_def(newSubProofPtr);
    ...
    for (step = plen - 1; step >= 0; step--) {
      ...
      if ((g_ProofInProgress.proof)[step] != repStatemNum
        ...
        newSubProofPtr = replaceStatement(repStatemNum,
            ...
            0, // noDistinct=0 OK since searchMethod=0 will only call proveFloating for $f's
...
#define MAX_GROWTH_FACTOR 2
    ...
    // break; // For special replacement with same label: always break to prevent inf loop
  } // end while
} // minimizeProof
...
/*E*/print2("step %ld stmt %ld\n",step,stmt);
  ...
  // Find the position in proof of all required hyps, and assign them.
...
char checkDummyVarIsolation(long testStep) { // 0=1st step, 1=2nd, etc.
  ...
  if (testStep == proofLen - 1) {
    // Dummy variables don't affect rest of proof
  ...
  for (hyp = g_Statement[parentStmt].numReqHyp - 1; hyp >= 0; hyp--) {
    // Skip computation at first loop iteration
...
{
  long proofLen, j;
  // First, make sure the output structure is empty to prevent memory leaks.
  deallocProofStruct(outProofStruct);

  // Get the proof length of the input structure
  proofLen = nmbrLen(inProofStruct.proof);
  // An empty proof should never occur here; proof should have at least
  // one step (possibly unknown).
    ...
    long proveStmt)
{
  ...
  // (Global) number of dummy (work) $nn variables, updated by function calls below
  g_pipDummyVars = 0; 
...
long processUndoStack(struct pip_struct *proofStruct,
    // PUS_INIT 1 Deallocates and initializes undo stack
    ...
    long newSize) // New maximum number of UNDOs for PUS_NEW_SIZE
{
  ...
  switch (action) {
    ...
    case PUS_PUSH:
      ...
      if (stackPtr == stackSize - 1) {
        ...
        for (i = 0; i < stackSize - 1; i++) {
          // Instead of "copyProofStruct(&(proofStack[i]), proofStack[i + 1]);
    ...
    case PUS_UNDO:
      // A first PUSH wasn't called upon entry to Proof Assistant (MM-PA)
