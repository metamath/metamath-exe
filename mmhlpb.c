...
H("section break), \"=-=-\" (for a subsection break), or \"-.-.\" (for a");
H("subsubsection break).  The line following that one that will be used for");
H("the table of contents entry, after trimming spaces.  In between two");
H("successive statements that generate web pages (i.e. $a and $p)");
H("statements), only the last of each break type will be used, and any");
H("smaller break before a larger break will be ignored.  See the set.mm");
