...
H("in the original RPN proof, you must SAVE PROOF <label> / NORMAL before");
H("using SHOW PROOF.");
H("");
...
H("        SHOW PROOF <label> / LEMMON / RENUMBER / NO_REPEATED_STEPS");
H("        will have the same proof step numbering as the web page proof");
H("        generated by SHOW STATEMENT  <label> / HTML, rather than");
H("        the proof step numbering of the indented format");
H("        SHOW PROOF <label> / RENUMBER.  This qualifier affects only");
...
H("version.]");
H("");
H("The output files will default to GIF format.  If a previous");
H("SHOW STATEMENT <label> / ALT_HTML was run, the output files will be in");
...
H("\"$( [dd-mmm-yyyy] $)\", where \"dd\" is 1 or 2 digits, e.g.");
H("\"$( [9-Oct-2012] $)\", with no leading zero.");
H("");
H("The output file will default to the GIF format for HTML.  If a previous");
H("SHOW STATEMENT <label> / ALT_HTML was run, the output files will be in");
H("Unicode format.  (A future version may add explicit / HTML and / ALT_HTML");
H("qualifiers.)");
H("");
...
H("        example, MINIMIZE_WITH ... / FORBID ax-ac will not shorten the");
H("        proof with any statement that depends on ax-ac.");
